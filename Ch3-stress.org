...
- At an internal point, the standard procedure is to calculate the tractions $\overset{1}{\boldsymbol{t}}, \overset{2}{\boldsymbol{t}}, \overset{3}{\boldsymbol{t}}$ that arise on the planes cornered by a Cartesian triad $\boldsymbol{e}_1, \boldsymbol{e}_2, \boldsymbol{e}_3$. 
  #+ATTR_HTML: :width 1600px
...
- The three traction vectors corresponding to ($+$) faces are  $\overset{1}{\boldsymbol{t}}, \overset{2}{\boldsymbol{t}}, \overset{3}{\boldsymbol{t}}$, and the other three associated with the ($-$) faces are $\overset{-1}{\boldsymbol{t}}, \overset{-2}{\boldsymbol{t}}, \overset{-3}{\boldsymbol{t}}$.
...
- Over planes that contain the load axis (whose normal will not not have an $\boldsymbol{e}_3$ component), *zero traction* previals!! That means $\overset{1}{\boldsymbol{t}}=\mathbf{0}$ and $\overset{2}{\boldsymbol{t}}=\mathbf{0}$, which yield the state of stress
...
*** Sample Problem 1: ([[./Ch2-vectors.html::#plane-normal][use plane formula]])
#+ATTR_HTML: :width 1600px
[[./images/FU94_3.3problem7.png]]

*** Sample Problem 2
#+ATTR_HTML: :width 1600px
[[./images/FU94_3.3problem8.png]]
...
- We used the rotation tensor definition $\beta_{i'j} = \boldsymbol{e}_{i'} \cdot \boldsymbol{e}_j$ for this purpose, and wrote the transformed coordinates as
  $$ x_{i'} = \beta_{i'j} x_j $$
...
#+ATTR_REVEAL: :frag (appear)  
- Thus, for a second-order tensor such as stress, two transformations are in order, as
  $$ \tau_{i'j'} = \beta_{i'i} \beta_{j'j} \tau_{ij}. $$
- Noting the order in which inner product takes place, the matrix form reveals
  $$ [\tau'] = [\beta] [\tau] [\beta]^T $$
...
*** Sample Problem 1
#+ATTR_HTML: :width 1600px
[[./images/FU94_3.4problem6.png]]

*** Sample Problem 2
#+ATTR_HTML: :width 800px
[[./images/fig3.4-problem1.jpg]]

#+REVEAL: split
#+ATTR_REVEAL: :frag (appear)
- Initial script produced in class
  #+BEGIN_SRC octave
    ...
    beta=[e_1p;e_2p;e_3p]
  #+END_SRC
- Two problems with this script can be identified
  #+ATTR_REVEAL: :frag (appear)
  1. the cylinder axis may not be aligned with $x_3$
  2. Segment BC may not be purely radial
- Corrected script     
  #+BEGIN_SRC octave
...
#+ATTR_REVEAL: :frag (appear)  
- condition of a body submerged in a fluid
  #+ATTR:REVEAL: :frag (appear)
  + a building subjected to wind
  + a vessel wall subjected to internal pressure
- condition at a bimaterial interface
  #+ATTR:REVEAL: :frag (appear)
  + relative sliding and contact between bodies
  + sticking interface (various joining methods exist)
- condition of point loads (singularities)
  + point forces and force couples
We know something about the forces or motion on the surface of a solid or a fluid and inquire what happens within ...

*** A Bimaterial Square Block Under Axial Contraction
#+ATTR_HTML: :width 1600px
[[./images/FU94_3.7figure11a.png]]
- Imagine the square is made of a soft upper half and a hard lower part joined together by the middle horizontal line.
#+ATTR_REVEAL: :frag (appear)
- Let the square block be compressed by the lateral side walls on the left and right. What will be the state of stress at the bimaterial interface?
- How will the state of stress change within each body?

#+REVEAL: split
#+ATTR_HTML: :width 1300px
[[./images/FU94_3.7figure11bcd.png]]
- From the interface, traction vector changes slightly within the soft part (a), and the hard part (b)
#+ATTR_REVEAL: :frag (appear)
- At the interface, boundary condition requires the /continuity of traction/, i.e. $\overset{i}{t}^{(1)} = \overset{i}{t}^{(2)}$.

*** What is external?
#+ATTR_HTML: :width 1600px
[[./images/FU94_3.7paragraph.png]]

** Sample Problem: Hydrostatic Stress
#+ATTR_HTML: :width 1600px
[[./images/FU94_3.7problem4.png]]
