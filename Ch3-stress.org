...
- Assume the load component in the axial-plane to be negligible?? (which is the same as saying the load is entirely axial and no shear component is taken)
  1. Using average values for a standard female, determine the normal compressive stress. (search keywords: CDC standard anthropometric measurements female data)
...
- A nominal stress calculation does not exist for this case (details about loading and boundary restraints should be provided to pose a /boundary-value problem/)
...
- Vasculatory system components, veins are pressurized biological tissues, but they operate under much lower pressure compared to arteries. Venous pressure is typically 5–8 mmHg
...
[[./images/fig1.11.7sphericalvessel.png]]  
#+ATTR_REVEAL: :frag (appear appear ...)  
- a *biaxial* state of stress endures, with the nominal hoop stress, $\sigma_\theta$, and nominal longitudinal stress, $\sigma_x$, which are not necessarily the same.
