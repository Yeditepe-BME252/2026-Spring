...
- To reduce vadue ideas into precise mathematical statements,
...
- These changes (deformation, etc.) together with axioms of continuum mechanics, can be expressed as boundary value problems
  $$ \sigma_{ij} = -p \delta_{ij} + \lambda D_{kk}\delta_{ij} + 2\mu D_{ij}\, \hspace{1em} \forall x \in \Omega $$
  \begin{align}
...
- good question... ask chatGPT\\
  _PROMPT:_ we are a group of undergraduate students enrolled in the biomedical engineering program. we are taking the biomechanics course, and it is the first time that we encounter the notion of a continuum. How could you give an everyday description of this notion to an ordinary person?
- Is the real number system a /continuum/? 
- What about in 3D? What is the most obvious reason for not being able to call *matter* as a continuum? 
...
#+ATTR_REVEAL: :frag (appear appear appear ...)
- Let the volume $\mathcal{V}_n$ be $V_n$ and the mass of the matter contained in $\mathcal{V}_n$ be $M_n$.
...
- /A material continuum is a/ definition for which /the densities of mass, momentum and energy exist in this strict mathematical sense/.
- No such materials exists!!

** Modifying the Limit to fit Reality
#+ATTR_REVEAL: :frag (appear appear appear ...)
- As $n \to \infty$, the limit of $V_n$ tends to a finite positive number $\omega$.
- The volume is not let to vanish but tends to a non-zero value controlled by a characteristic size (e.g. on the order of $\mathcal{O}(\mu m)$) below which the continuity of the distribution is disrupted.
...
#+ATTR_REVEAL: :frag (appear appear appear ...)
- Imagine we obtain the material properties (e.g. density, etc.) for a Real Continuum.
...
#+ATTR_REVEAL: :frag (appear appear appear ...)
- Example is from the human lung. Three consecutive trees, an airway tree, an arterial three and a venous tree.
  
** Questions for next session
Ask AI:
- Discuss the axioms of mechanics.
